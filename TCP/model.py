...
class TCP(nn.Module):
	...
	def forward(self, gt, img, state, target_point, embedding, vlm_output_type,
	...
	def evaluate(self, img, state, target_point, embedding):
		feature_emb, cnn_feature = self.perception(img)
		outputs = {}
		outputs['pred_speed'] = self.speed_branch(feature_emb)
		measurement_feature = self.measurements(state)
